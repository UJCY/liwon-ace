...
회귀 2축(D7)과 **같은 형식**으로 잰다. 하나의 점수로 합치지 않는다.
...
| `OP-04` | **시간 + 사건 + 서술 복합** — question-taxonomy 10.2 (3)이 지적한 "최근"이 걸리는 지점. 구조상 X3 정의와 일치한다 | `[nl2sql, vector_search]` | `parallel_merge` (`temporal`) |
