...
> 정리 시점(2026-07-30) 최신 리비전: **2026-07-28**. 권위 정의는 TypeScript 스키마(schema.ts).
...
| 도구 이름 규칙 | `ask`, `nl2sql`, `vector_search`, `knowledge_graph` 모두 규칙 부합 |
...
| 리비전 확인 | 선택할 SDK/air 프레임워크가 지원하는 스펙 리비전 확인 — 스택 결정(열린 항목)의 검증 항목에 추가 |
