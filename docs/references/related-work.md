...
## 4.10 "MCP Parallel 패턴" 용어 소재 — 가설 (우리 판단 · 미검증)
...
**요강 원문이 저장소에 없다.** 요강에 다른 패턴 이름(Prompt chaining, Orchestrator-workers 등)이 함께 등장하는지 확인되면 이 가설은 거의 확정되는데, 지금은 손이 닿지 않는다. `dataset-analysis.md` 6장의 "전문 확보" 전수 검사도 같은 이유로 재현 불가다 — 블로그는 curl 스니펫이 있지만 요강은 없다. **요강 원문 아카이빙이 이 가설의 결정적 검증 수단이다.**
