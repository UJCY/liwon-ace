...
**남은 확인 수단**: 주최 측 문의 — 요강 기술문의처 `sihyeon@liwonace.co.kr` (연구원 이시현). 물어보면 끝나는 문제다. **다른 문의 후보였던 "LLM 모델"(Gemma 4 E2B 대 KOSSA "7B") 행은 2026-08-14에 문의 대상에서 빠졌다** — 리원에이스 2출처가 동일 문면이라 문의로 얻을 정보가 없다고 판단했다 ([design.md](../design.md) 열어둔 항목). **대신 2026-08-18에 새 문의 후보가 하나 생겼다 — "RAM 4GB 하한 대응"이다.** CPU 전용 실측에서 권장 모델이 단독 4.0GB로 나왔는데, 원문의 `RAM 최소 4GB`는 블로그 한 곳의 *"권장 개발 환경"*(=개발 기계) 표에만 있고 **심사 실행기 사양을 적은 문면이 어디에도 없다**. 문면으로는 더 좁힐 수 없어 같은 창구로 물어야 한다 ([design.md](../design.md) 열어둔 항목, [dataset-analysis.md](../dataset-analysis.md) 7장). **따라서 이 창구의 문의 후보는 이 절의 용어 문제와 심사 환경 사양 둘이다.**
