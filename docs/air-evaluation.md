...
**공식 SDK 자체가 2026-07-28 리비전을 아직 구현하지 않았다.** 즉 air를 쓰든 SDK를 직접 쓰든 우리가 말할 수 있는 최신 리비전은 `2025-11-25`이며, 이는 프레임워크 선택과 무관한 생태계 상태다. ([mcp-spec.md](./references/mcp-spec.md)의 "최신 리비전 2026-07-28"은 **스펙 문서상** 최신이고 구현은 뒤처져 있다 — 해당 문서에 교정 표기함.)
