...
#23은 라우팅이 맞더라도 실행하면 개체 부재(T5)가 된다. **#23의 정답을 "결과 반환"이 아니라 "개체 부재(T5) 응답"으로 재정의한 근거다** ([design.md](./design.md) D7). 재정의에 문항을 읽는 일이 필요하다는 점이 회귀 실행 상태를 라벨 없이 관측만 하기로 한 근거이기도 하다 ([design.md](./design.md) D18).
