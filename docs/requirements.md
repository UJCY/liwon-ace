...
3. **도구 자동 선택** — 블로그 원문: *"규칙 기반 라우터가 질문 유형을 분석하여 **적합한 도구를 자동 매칭**합니다."* (단수, "매칭"이지 "호출"이 아님). KOSSA 요강은 이를 "MCP Parallel 패턴"이라 부름 — 아래 주의 참조.
...
**어느 출처도 무엇이 병렬인지 설명하지 않는다.** 유일한 출현은 "규칙 기반 라우터를 통한 도구 자동 선택"에 붙은 괄호 별칭이다. 반면 블로그는 라우터를 **단수 매칭**으로 서술하고, questions.json 30개도 전부 도구 1개다.
