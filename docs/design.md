...
### D1. 라우터는 서버 측 게이트웨이 도구 (`ask`)

에이전트가 아니라 MCP 서버 안에 라우터를 둔다. `ask(question)` 도구가 규칙 기반으로 질문 유형을 분석해 내부에서 도구를 선택·호출한다.

- **이유**: 데이터셋 README의 도구 매핑 표가 라우터를 "MCP 도구"로 표기. 소형 LLM에게 도구 선택을 맡기지 않으므로 (규칙 기반 요구) 선택 로직이 서버에 있는 편이 에이전트를 단순하게 유지. MCP Parallel 패턴(병렬 호출·병합)도 서버 내부 구현이 자연스러움.
- **기각한 대안**: 에이전트 측 라우터 모듈 (과제 흐름도와는 더 유사하나 에이전트가 복잡해짐), LLM 도구 선택 + 규칙 폴백 ("규칙 기반" 문면과 어긋남).

### D2. 등록 도구는 4개 — `ask` + 3종

`ask`, `vector_search`, `nl2sql`, `knowledge_graph` 전부 MCP 도구로 등록. 에이전트는 평소 `ask`만 호출하고, 3종은 개별 검증·시연·심사용으로 직접 호출 가능하게 열어둔다.
